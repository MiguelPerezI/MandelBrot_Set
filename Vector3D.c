...
Vector3D reflectRespectPoint(Vector3D a, Vector3D auxReflection) {
	return add(auxReflection, scale(-1.0, sub(a, auxReflection)));
}

Vector3D dilate(Vector3D a, Vector3D auxDilate, double lambda) {
	return add(auxDilate, scale(lambda, sub(a, auxDilate)));
}

Vector3D centerSquare(Vector3D a, Vector3D b, Vector3D c, Vector3D d) {
	return scale(0.25, add(d, add(c, add(a, b))));
}